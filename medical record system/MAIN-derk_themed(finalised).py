...
def savedata():
    ...
    try:
        ...
        winsound.PlaySound('voices/record has been inserted.wav',0)
    except:
        messagebox.showerror('duplicate error','Record already exists')
        winsound.PlaySound('voices/record already exists.wav',0)
...
def dlt():
    ...
    def disp():
        ...
        try:
            ...
            winsound.PlaySound('voices/no records found.wav',0)
    ...
    def yo():
        try:
            ...
            winsound.PlaySound('voices/record has been deleted.wav',0)
        except:
            messagebox.showerror('OOPS','NO RECORD')
            winsound.PlaySound('voices/no records found.wav',0)
...
def modify():
    ...
    def sd():
        ...
        try:
            ...
            winsound.PlaySound('voices/record has ben updated.wav',0)
        except:
            messagebox.showerror('Error',"Record couldn't be edited , Recheck entry")
            winsound.PlaySound("voices/record couldn't be updated.wav",0)
def view():
    admn=IntVar()
    import tkinter as tk
    cursor = mydb.cursor()
...
def login():
    ...
    admin.configure(bg='#181818')
    l=Label(admin,text='',bg='#181818').grid(row=0,column=0)
    lab1=Label(admin,text='   USERNAME :',font=(fn,15),bg='#181818',fg='white').grid(row=1,column=0)
    lab2=Label(admin,text='   PASSWORD :',font=(fn,15),bg='#181818',fg='white').grid(row=2,column=0)
    ent1=Entry(admin,bg='#181818',fg='white')
    ent1.grid(row=1,column=1)
    ent2=Entry(admin,show='•',bg='#181818',fg='white')#•●
    ent2.grid(row=2,column=1)
    l=Label(admin,text='',bg='#181818').grid(row=3,column=0)
    b1=Button(admin,text='LOGIN',command=log)
...
def log():
    ...
    if a=='admin' and b=='admin':
        ...
        winsound.PlaySound('voices/incorrect password or username.wav',0)
    
root=Tk()
root.geometry('600x470')
##with open('medi4.png','rb')as img_file:
    #my_ing=base64.b64encode(img_file.read())
bg=PhotoImage(file='imagez/medi4.png')
root.configure(bg='#181818')
root.title('MEDICAL RECORDS')
icoon=PhotoImage(file='imagez/cross.png')
...
lab2=Label(root,text='M E D I C A L   R E C O R D S   M A I N T E N A N C E',font=('Agency fb bold',19),bg='#181818',fg='white')
...
f1=Frame(root)
f1.pack(pady=20)
f1.configure(bg='#181818')
b1=Button(f1,text='ENTER',command=lambda:[mini(),login()],width=10)
b1.pack(pady=4)
b2=Button(f1,text='EXIT',command=lambda:[root.destroy()],width=10)
b2.pack(pady=4)
